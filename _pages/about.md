---
permalink: /
title: "Isen Naiken"
excerpt: "Multimodal AI for Cancer and the Regulatory and Entrepreneurial pathway."
author_profile: true
header:
  overlay_color: "#1a2332"
  overlay_filter: "0.6"
redirect_from: 
  - /about/
  - /about.html
---

Doctor of Pharmacy (PharmD) and PhD candidate in Computer Science & Artificial Intelligence, I work at the intersection of clinical pharmacy and advanced AI models. My research focuses on predicting CAR-T cell response using reliable multimodal AI in patients with Diffuse Large B-Cell Lymphoma (DLBCL), supervised within the Computer Science department at the University of Burgundy.

My professional trajectory combines academic research with entrepreneurship through **Nexomedis®**, a startup I co-founded to facilitate the technology transfer of AI solutions into clinical practice. I also serve as **President & Editor-in-Chief** of *Revue IA Santé®*, a peer-reviewed journal dedicated to AI in healthcare promoting open science and reproducible research.

Research Interests
======
- Multimodal AI and deep learning for clinical oncology
- Computational pathology (Whole Slide Images, Vision Transformers)
- Biomarker discovery using small extracellular vesicles (sEVs)
- Large Language Models applied to biomedical literature
- Reliable and interpretable AI for healthcare

Current Positions
======
- **PhD Candidate** – Computer Science & AI, University of Burgundy (2024–Present)
- **Co-Founder & CSO** – Nexomedis® (2024–Present)
- **President & Editor-in-Chief** – Revue IA Santé® (2024–Present)

Selected Awards
======
- **Prize** – ESEO Datathon 2025 (ADLIS Challenge): Deep Learning algorithm for automated ring sideroblast identification
- **"Les Entrep' 2025" Award**: Top innovation prize for the Nexomedis project
- **"Young Researcher" Award** – Cancéropôle Est
- **Deeptech Grant** – Appel à projet Deeptech (Nexomedis)
- **DECA-BFC Incubator Grant** – Regional incubator program, Bourgogne-Franche-Comté

Education
======
- M.Sc. in Health & Artificial Intelligence – Research Track, University of Burgundy, 2025
- Doctor of Pharmacy (PharmD), University of Burgundy, 2024
- National Hospital Pharmacy & Medical Biology Residency Exam (*Concours de l'Internat en Pharmacie*), ranked 345th, 2023
- M.Sc. in Pharmaceutical Sciences (*Diplôme de Formation Approfondie en Sciences Pharmaceutiques*), University of Burgundy, 2017–2023
- Dual Degree: M.Sc. Year 1 in CNS Diseases & Treatments, 2022
- Competitive First Common Year of Health Studies Exam (PACES), passed, 2019
- Scientific Baccalaureate, European Section, mention *Très Bien*, 2018
